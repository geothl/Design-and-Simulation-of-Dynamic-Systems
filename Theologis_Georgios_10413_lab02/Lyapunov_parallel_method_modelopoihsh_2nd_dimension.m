function dynamics = Lyapunov_parallel_method_modelopoihsh_2nd_dimension(dyn,u,A,B,Gamma)
xdot=A*[dyn(1);dyn(2);]+B*u;
dynamics(1)=xdot(1);
dynamics(2)=xdot(2);
A_ek=[dyn(5) dyn(6);dyn(7) dyn(8);];
B_ek=[dyn(9);dyn(10)];
xdot_ek=A_ek*[dyn(3); dyn(4);]+B_ek*u;
dynamics(3)=xdot_ek(1);
dynamics(4)=xdot_ek(2);
A_ek_dot=Gamma(1)*([dyn(1);dyn(2);]-[dyn(3);dyn(4);])*transpose([dyn(3);dyn(4);]);
